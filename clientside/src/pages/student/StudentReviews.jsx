...
//       const response = await fetch(`${BASE_URL}/reviews/general/stats`, {
//         credentials: "include",
//       });
...
//       }
//     } catch {
//       // Silent fail - stats will be null
//     }
//   };
...
const StudentReviews = () => {
  ...
  const fetchMyReviews = async () => {
    try {
      const response = await fetch(`${BASE_URL}/reviews/my-reviews`, {
        credentials: "include",
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingReview
        ? `${BASE_URL}/reviews/general/${editingReview.id}`
        : `${BASE_URL}/reviews/general`;
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      type="submit"
                      disabled={submitting || rating === 0}
                      className="flex-1 px-4 py-2.5 bg-teal-600 hover:bg-teal-700 disabled:bg-gray-400 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
                      <Send size={18} />
                      {submitting
                        ? "Submitting..."
                        : editingReview
  ...
  // Admin View (keeps existing admin functionality with overall ratings)
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <p className="text-gray-600 text-sm">
            View all user reviews and overall portal ratings.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Overall Rating Card - Admin Only */}
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">
                Overall Rating
              ...
              <div className="space-y-6">
                {reviews.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-sm">
                      Waiting for users to share their feedback.
                    </p>
                  </div>
                ) : (
                  reviews.map((review) => (
